    ...
    public class CostCentersStoredProcedures : IStoredProcedures
    {
        ...
        private void GetAllData()
        {
            if (!Helper.StoredProcedureExists($"dbo.{TableName}_GetAll", DatabaseNames.FinancialAnalysisDB))
            {
                ...
                sbSP.AppendLine($"CREATE PROCEDURE [{TableName}_GetAll] AS BEGIN SET NOCOUNT ON; " +
                                "SELECT CostCenterId, Name, Description " +
                                $"FROM {TableName} " +
                                "END");
        ...
        private void GetById()
        {
            if (!Helper.StoredProcedureExists($"dbo.{TableName}_GetById", DatabaseNames.FinancialAnalysisDB))
            {
                ...
                sbSP.AppendLine(
                    $"CREATE PROCEDURE [{TableName}_GetById] @CostCenterId int AS BEGIN SET NOCOUNT ON; SELECT CostCenterId, Name, Description " +
                    $"FROM {TableName} " +
        ...
        private void UpdateData()
        {
            if (!Helper.StoredProcedureExists($"dbo.{TableName}_Update", DatabaseNames.FinancialAnalysisDB))
            {
                ...
                sbSP.AppendLine(
                    $"CREATE PROCEDURE [{TableName}_Update] @CostCenterId int, @Name nvarchar(150), @Description nvarchar(MAX) " +

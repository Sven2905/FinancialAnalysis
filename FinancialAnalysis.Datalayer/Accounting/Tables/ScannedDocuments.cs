...
namespace FinancialAnalysis.Datalayer.Accounting
{
    ...
    {
        private readonly ScannedDocumentsStoredProcedures sp = new ScannedDocumentsStoredProcedures();
        ...
        public int Insert(ScannedDocument scannedDocument)
        {
            ...
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    var result = con.Query<int>($"dbo.{TableName}_Insert @Content, @FileName, @Date, @RefBookingId",
                        scannedDocument);
        ...
        public void Insert(IEnumerable<ScannedDocument> scannedDocuments)
        {
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    foreach (var scannedDocument in scannedDocuments) Insert(scannedDocument);
                }
            }
            catch (Exception e)
            {
        ...
        {
            var output = new ScannedDocument();
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    output = con.QuerySingleOrDefault<ScannedDocument>($"dbo.{TableName}_GetById @ScannedDocumentId",
                        new {ScannedDocumentId = id});
                }
            }
            catch (Exception e)
            {
        ...
        {
            foreach (var scannedDocument in scannedDocuments) UpdateOrInsert(scannedDocument);
        }
        ...
        {
            if (scannedDocument.ScannedDocumentId == 0 || GetById(scannedDocument.ScannedDocumentId) is null) return;
            ...
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    con.Execute($"dbo.{TableName}_Update @ScannedDocumentId, @Content, @FileName, @Date, @RefBookingId",
                        scannedDocument);
                }
            }
            catch (Exception e)
        ...
        private void AddBookingsReference()
        {
            try
            {
                var con = new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB));
                var commandStr =
                    $"IF(OBJECT_ID('FK_ScannedDocuments_Bookings', 'F') IS NULL) ALTER TABLE {TableName} ADD CONSTRAINT FK_ScannedDocuments_Bookings FOREIGN KEY(RefBookingId) REFERENCES Bookings(BookingId)";

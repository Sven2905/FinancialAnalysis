...
namespace FinancialAnalysis.Datalayer.ProjectManagement
{
    public class ProjectRoles : ITable
    {
        ...
        public void Insert(IEnumerable<ProjectRole> ProjectRoles)
        {
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    foreach (var ProjectRole in ProjectRoles) Insert(ProjectRole);
                }
            }
            catch (Exception e)
            {
        ...
        {
            var output = new ProjectRole();
            try
            {
                using (IDbConnection con =
                    new SqlConnection(Helper.GetConnectionString(DatabaseNames.FinancialAnalysisDB)))
                {
                    output = con.QuerySingleOrDefault<ProjectRole>($"dbo.{TableName}_GetById @ProjectRoleId",
                        new {ProjectRoleId = id});
                }
            }
            catch (Exception e)
            {
